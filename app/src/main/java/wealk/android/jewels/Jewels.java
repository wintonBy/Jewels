...
public class Jewels extends BaseGameActivity implements IOnSceneTouchListener, IConstants {
	...
	public void onLoadResources() {
		...
		this.mScoreFont = FontFactory.createFromAsset(this.mScoreFontTexture, this, "fonts/winton.ttf", 44, true, Color.WHITE);
		...
		this.mChapterFont = FontFactory.createFromAsset(this.mChapterTexture, this, "fonts/winton.ttf", 44, true, Color.GREEN);
...
//				}
	...
	private void removeHorizontal(){
    	...
    	for(int i = 0; i < CELLS_VERTICAL; i++)
    	{
    		...
    		{
    			currentStyle = mHashMap.get(getKey(j,i)).getStyle();
	...
	private void removeVrtical(){
    	...
    	for(int i = 0; i < CELLS_HORIZONTAL; i++)
    	{
    		...
    		{
				currentStyle = mHashMap.get(getKey(i,j)).getStyle();
	...
	private void startNewChapter(){
		spViewStyle = MathUtils.random(0, 6);
		System.out.println(spViewStyle+"特殊的sp");
		Log.d("Winton", "我是spView"+spViewStyle);
	 ...
	 * @param fallCount
	 */
	private void addScore(int fallCount,int currentStyle){
		switch (fallCount) {
		case 3:
			if(currentStyle == spViewStyle){
				Log.d("Winton","我是新的分数");
				this.mScore += 40;
				break;
			}
